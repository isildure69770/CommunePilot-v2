...
interface IdentityValue { ready: boolean; azureDeployment: boolean; authenticated: boolean; user: CommuneUser; users: CommuneUser[]; setCurrentUser(id: string): void; refreshUsers(): void; can(domain: PermissionDomain, action?: PermissionAction): boolean; }
...
export function LocalIdentityProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = useMemo<IdentityValue>(() => ({ ready: azureAuthentication.status !== "loading", azureDeployment: azureAuthentication.status !== "local", authenticated: azureAuthentication.status === "local" || azureAuthentication.status === "authenticated", user, users: availableUsers, refreshUsers, setCurrentUser(id) { if (azureUser) return; localStorage.setItem(PROFILE_KEY, id); setCurrentId(id); }, can: (domain, action = "view") => can(user.role, domain, action) }), [azureAuthentication.status, azureUser, availableUsers, user]);
