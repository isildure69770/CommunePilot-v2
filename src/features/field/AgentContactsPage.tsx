...
const electedRoles = new Set(["Maire", "Adjoint", "Conseiller"]);

export default function AgentContactsPage() {
  const { users } = useIdentity();
  const contacts = users.filter((user) => user.active && electedRoles.has(user.role));
  return <section className="agent-contacts-page">
    ...
    <div className="agent-contact-list">{contacts.map((contact) => <article key={contact.id}>
      <span className="agent-contact-avatar"><UserRound/></span>
      <div><strong>{contact.firstName} {contact.lastName}</strong><small>{contact.role}</small></div>
