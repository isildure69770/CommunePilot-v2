...
import { AlertTriangle, Bell, Camera, Check, CheckCircle2, ChevronLeft, ChevronRight, ClipboardList, Clock3, FileText, Flag, History, Info, Map, MapPin, Navigation, Play, X, ZoomIn, ZoomOut } from "lucide-react";
...
export default function TerrainPage() {
  ...
  return <section className="terrain-page">
    ...
    <button className="agent-alert-history-link" type="button" onClick={() => setSection("alerts")}><span><History/><strong>Alertes de tous les agents</strong><small>{sharedAlerts.length} alerte{sharedAlerts.length > 1 ? "s" : ""} · {sharedAlerts.filter((alert) => alert.status === "Nouveau").length} à prendre en charge</small></span><ChevronRight/></button>
    ...
    {section === "alerts" && <section className="terrain-alerts-view agent-alert-history"><header><div><span className="eyebrow">File terrain partagée</span><h3>Alertes de tous les agents</h3><p>Consultez et traitez les signalements de toute l’équipe.</p></div><button type="button" onClick={() => setSearchParams({ "nouvelle-alerte": "1" })}><AlertTriangle/> Nouvelle alerte</button></header>{sharedAlerts.length > 0 ? <div>{sharedAlerts.map((alert) => { const author = users.find((candidate) => candidate.id === alert.createdBy); const handler = users.find((candidate) => candidate.id === alert.handledBy); const canComplete = alert.status === "Pris en compte" && alert.handledBy === user.id; const priority = alert.priority || "Normale"; const photos = alert.photos || []; const createdAt = alert.createdAt ? new Date(alert.createdAt).toLocaleString("fr-FR", { dateStyle: "medium", timeStyle: "short" }) : "Date inconnue"; return <article key={alert.id}><span className={`alert-priority priority-${priority.toLowerCase()}`}>{priority}</span><div className="alert-history-heading"><strong>{alert.category || "Autre"}</strong><em>{alert.status || "Nouveau"}</em></div><time>{createdAt}</time><p>{alert.comment || "Aucune description"}</p><small><MapPin/>{alert.address || (alert.latitude != null ? "Position GPS transmise" : "Localisation non précisée")}</small><span className="alert-history-author">Créée par {author ? `${author.firstName} ${author.lastName}` : "un agent"}{handler && ` · Prise par ${handler.firstName}`}</span>{photos.length > 0 && <div className="alert-history-photos">{photos.slice(0, 4).map((photo, index) => <button type="button" key={photo.id} onClick={() => showGallery(photos, index)}><img src={photo.dataUrl} alt={photo.name}/></button>)}</div>}{(!alert.status || alert.status === "Nouveau") && <button className="alert-take-button" type="button" onClick={() => takeAlert(alert)}><Check/>Prendre en charge</button>}{canComplete && <button className="alert-complete-button" type="button" onClick={() => completeAlert(alert)}><CheckCircle2/>Marquer comme traitée</button>}{alert.status === "Pris en compte" && alert.handledBy !== user.id && <span className="alert-history-locked">Déjà prise en charge par {handler?.firstName || "un autre agent"}</span>}{alert.missionId && <span className="alert-history-linked"><CheckCircle2/> Transformée en mission</span>}</article>; })}</div> : <div className="empty-state"><AlertTriangle/><strong>Aucune alerte envoyée</strong><span>Utilisez le bouton « Nouvelle alerte » pour prévenir les élus.</span></div>}</section>}
...
function AlertForm({ onClose, onSubmit, userId }: { onClose(): void; onSubmit(value: FieldAlert): void; userId: string }) {
  ...
  const send = () => {
    ...
    catch { setSendError("L’alerte n’a pas pu être enregistrée. Réessayez avec moins de photos."); }
  };
