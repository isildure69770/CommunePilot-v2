...
import { AlertTriangle, Camera, Check, CheckCircle2, ChevronLeft, ChevronRight, ClipboardList, ContactRound, FileText, Flag, Map, MapPin, Navigation, Play, Trash2, X, ZoomIn, ZoomOut } from "lucide-react";
...
import MissionLocationPreview from "./MissionLocationPreview";
...
export default function TerrainPage() {
  ...
  const closeAlertsHub = () => { setAlertsHubOpen(false); if (searchParams.has("mes-alertes")) setSearchParams({}); };
  ...
  useEffect(() => { if (searchParams.get("mes-alertes") === "1") setAlertsHubOpen(true); }, [searchParams]);

  return <section className="terrain-page">
    <header className="agent-home-heading"><div><h2>Bonjour {user.firstName} <span aria-hidden="true">👋</span></h2><p>Agent technique</p></div></header>
