...
export default function TerrainPage() {
  ...
  return <section className="terrain-page">
    ...
    <button className="agent-alert-history-link" type="button" onClick={() => setSection("alerts")}><span><History/><strong>Historique de mes alertes</strong><small>{myAlerts.length} alerte{myAlerts.length > 1 ? "s" : ""} créée{myAlerts.length > 1 ? "s" : ""}</small></span><ChevronRight/></button>
    ...
    {section === "alerts" && <section className="terrain-alerts-view agent-alert-history"><header><div><span className="eyebrow">Historique personnel</span><h3>Mes alertes créées</h3><p>Suivez ici leur prise en compte par les responsables.</p></div><button type="button" onClick={() => setSearchParams({ "nouvelle-alerte": "1" })}><AlertTriangle/> Nouvelle alerte</button></header>{myAlerts.length > 0 ? <div>{myAlerts.map((alert) => <article key={alert.id}><span className={`alert-priority priority-${alert.priority.toLowerCase()}`}>{alert.priority}</span><div className="alert-history-heading"><strong>{alert.category}</strong><em>{alert.status}</em></div><time>{new Date(alert.createdAt).toLocaleString("fr-FR", { dateStyle: "medium", timeStyle: "short" })}</time><p>{alert.comment}</p><small><MapPin/>{alert.address || (alert.latitude != null ? "Position GPS transmise" : "Localisation non précisée")}</small>{alert.photos.length > 0 && <div className="alert-history-photos">{alert.photos.slice(0, 4).map((photo, index) => <button type="button" key={photo.id} onClick={() => showGallery(alert.photos, index)}><img src={photo.dataUrl} alt={photo.name}/></button>)}</div>}{alert.missionId && <span className="alert-history-linked"><CheckCircle2/> Transformée en mission</span>}</article>)}</div> : <div className="empty-state"><AlertTriangle/><strong>Aucune alerte envoyée</strong><span>Utilisez le bouton « Nouvelle alerte » pour prévenir les élus.</span></div>}</section>}
