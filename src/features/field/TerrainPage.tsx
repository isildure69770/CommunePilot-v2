...
export default function TerrainPage() {
  ...
  return <section className="terrain-page">
    <header className="terrain-heading"><div><span className="eyebrow">Espace terrain</span><h2>Bonjour {user.firstName}</h2><p>{mine.filter((item) => !["Terminée"].includes(item.status)).length} mission(s) à réaliser</p><SyncIndicator status={alertSync.status} error={alertSync.error} onSync={() => void synchronizeAlerts()}/></div></header>
